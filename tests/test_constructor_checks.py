...
@hyp.given(smoothing_factor = st.one_of(
...
def test_cannot_construct_without_global_error_tolerance_set_and_without_max_iterations(dummy_args):
    with pytest.raises(InvalidParameter):
        IDESolver(
            *dummy_args,
            global_error_tolerance = 0,
...
def test_can_construct_with_positive_global_error_tolerance(dummy_args, global_error_tolerance):
    hyp.assume(global_error_tolerance > 0)  # this test does not cover the case where tol = 0 and max_iterations = None

    IDESolver(
        *dummy_args,
        global_error_tolerance = global_error_tolerance,
    )
