...
def test_y_intermediate_list_exists_if_store_intermediate_y_is_true() -> None:
    solver = IDESolver(
        x=np.linspace(0, 1, 100),
        y_0=0,
        c=lambda x, y: y - (0.5 * x) + (1 / (1 + x)) - np.log(1 + x),
...
def test_number_of_intermediate_solutions_is_same_as_iteration_count_plus_one() -> None:
    solver = IDESolver(
        x=np.linspace(0, 1, 100),
        y_0=0,
        c=lambda x, y: y - (0.5 * x) + (1 / (1 + x)) - np.log(1 + x),
...
def test_intermediate_solutions_of_scalar_problem_is_list_of_scalar_arrays() -> None:
    solver = IDESolver(
        x=np.linspace(0, 1, 100),
        y_0=0,
        c=lambda x, y: y - (0.5 * x) + (1 / (1 + x)) - np.log(1 + x),
...
def test_intermediate_solutions_of_vector_problem_is_list_of_vector_arrays() -> None:
    solver = IDESolver(
        x=np.linspace(0, 1, 100),
        y_0=[0, 1, 0],
...
def default_solver() -> IDESolver:
    return IDESolver(x=np.linspace(0, 1, 100), y_0=0)


@hyp.given(x=st.complex_numbers(), y=st.complex_numbers())
def test_default_c(default_solver: IDESolver, x: complex, y: complex) -> None:
    assert default_solver.c(x, y) == 0


@hyp.given(x=st.complex_numbers())
def test_default_d(default_solver: IDESolver, x: complex) -> None:
    assert default_solver.d(x) == 1


@hyp.given(x=st.complex_numbers(), s=st.complex_numbers())
def test_default_k(default_solver: IDESolver, x: complex, s: complex) -> None:
    assert default_solver.k(x, s) == 1


@hyp.given(y=st.complex_numbers())
def test_default_f(default_solver: IDESolver, y: complex) -> None:
    assert default_solver.f(y) == 0


def test_default_lower_bound(default_solver: IDESolver) -> None:
    assert default_solver.lower_bound(default_solver.x) == default_solver.x[0]


def test_default_upper_bound(default_solver: IDESolver) -> None:
